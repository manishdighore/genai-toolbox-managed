...
func ServeFlags(flags *pflag.FlagSet, opts *ToolboxOptions) {
	...
	flags.StringVar(&opts.Cfg.DBURL, "db-url", "", "Postgres DSN for the management DB, e.g. postgres://user:pass@host:5432/dbname?sslmode=require. Required when --config-mode=db.")
	flags.StringVar(&opts.Cfg.SecretsBackend, "secrets-backend", "credentials", "Secrets backend: 'credentials' (Postgres credentials table, default), 'gcp', 'aws', or 'azure'.")
	flags.StringVar(&opts.Cfg.AppKey, "app-key", "", "32-byte hex master key for envelope-encrypting rows in the credentials table (credentials backend only). Generate with: openssl rand -hex 32")
	flags.StringVar(&opts.Cfg.CredentialsOwner, "credentials-owner", "", "owner_id stamped on rows db-mcp writes to the credentials table (credentials backend only). Default: 'db-mcp'.")
